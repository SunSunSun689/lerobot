...
class LeRobotDataConverter:
    ...
    def convert_dataset(self):
        ...
        # 计算每个 episode 在 chunk 视频中的全局帧偏移（同一 chunk 内按顺序累加）
        chunk_frame_offset = {}  # chunk_name -> 当前已累计帧数
        episode_frame_offsets = {}  # episode_idx -> (chunk_name, global_start_frame)
        for episode_idx, episode_df, chunk_name, _ in sorted(all_episodes, key=lambda x: x[0]):
            offset = chunk_frame_offset.get(chunk_name, 0)
            episode_frame_offsets[episode_idx] = (chunk_name, offset)
            chunk_frame_offset[chunk_name] = offset + len(episode_df)

        # 转换每个episode
        for episode_idx, episode_df, chunk_name, parquet_file in all_episodes:
            episode_name = f"episode_{episode_idx:06d}"
            print(f"\n处理 {episode_name}...")
            _, global_start_frame = episode_frame_offsets[episode_idx]
            self.convert_episode(
                episode_df, episode_name, episode_idx, chunk_name, parquet_file,
